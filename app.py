...
def chat_with_llm(e2b_code_interpreter: Sandbox, user_message: str, dataset_path: str) -> Tuple[Optional[List[Any]], str]:
    # Update system prompt to include dataset path information and code generation instructions
...
def upload_dataset(code_interpreter: Sandbox, uploaded_file) -> str:
    ...
    try:
        # Write the actual bytes of the uploaded file to the sandbox
...
def main():
    ...
    # Step 1: Human Intelligence Capture
    st.header("Step 1: Human Intelligence Capture 🧠")
    human_intuition = st.text_area(
        "Before AI analyzes your data, what do you *feel* this data will show? Any human factors, emotions, or cultural context to consider?",
        placeholder="E.g., I think sales dip in March because of post-holiday stress..."
    )
        ...
        if st.button("Analyze"):
            if not st.session_state.together_api_key or not st.session_state.e2b_api_key:
                ...
                with Sandbox(api_key=st.session_state.e2b_api_key) as code_interpreter:
                    ...
                    # Step 2 & 3: Display Human vs AI Insights Side-by-Side
                    st.markdown("---")
                    st.header("Step 2: Human vs AI Insights 🧠🤖")
                    col1, col2 = st.columns(2)
                    with col1:
                        st.subheader("🧠 Human Intuition")
                        if human_intuition.strip():
                            st.write(human_intuition)
                        else:
                            st.write(":grey[No human intuition provided.]")
                    with col2:
                        st.subheader("🤖 AI Analysis")
                        st.write(llm_response)

                    # Step 4: Display results/visualizations
                    st.markdown("---")
                    st.header("Step 3: Visualizations & Results 📊")
                    if code_results:
                        ...
                        st.info("No visualizations or results were returned by the AI code.")
